...
class ViewController: UIViewController {
    ...
    func setupGestures() {
        let tapGesturRecognizer = UITapGestureRecognizer(target: self, action: #selector(placeVirtualObject(tapGesture:)))
        tapGesturRecognizer.numberOfTapsRequired = 1
        self.sceneView.addGestureRecognizer(tapGesturRecognizer)
        
        
      // ДЛя 2 касаний задержка   tapGesturRecognizer.require(toFail: doubletapGesturRecognizer)
    }
    ...
    @objc func placeVirtualObject(tapGesture: UITapGestureRecognizer) {
        ...
        creatVirtualObject(hitResult: hitResult)
        
        
    }
    
    func creatVirtualObject(hitResult: ARHitTestResult) {
